...
export async function generateMetadata({ searchParams }) {
  ...
  let title = 'Tất cả sản phẩm | Nha Khoa Smile';
  if (categorySlug) {
    const cats = await query('SELECT name FROM shop_categories WHERE slug = ?', [categorySlug]);
    if (cats.length > 0) {
      title = `${cats[0].name} | Nha Khoa Smile`;
    }
  } else if (searchQ) {
    title = `Tìm kiếm: "${searchQ}" | Nha Khoa Smile`;
  }
  
  return {
    title,
    description: 'Dịch vụ nha khoa chuẩn quốc tế: trồng răng Implant, niềng răng thẩm mỹ, răng sứ, tẩy trắng răng tại Nha Khoa Smile.'
  };
}
